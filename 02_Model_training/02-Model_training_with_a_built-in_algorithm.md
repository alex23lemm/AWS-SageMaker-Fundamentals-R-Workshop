# SageMaker fundamentals for R users <br/> Part 02: Training a model with a built-in algorithm
...
*Note:* We will store the test set to disk again, this time including the dependent variable. We will use this dataset in the third workshop module, [Part 03: Hyperparameter tuning](/03_Model_tuning/03-Model_tuning.md). In addition,  We will store just the table header to CSV. We will use the saved header information in the fifth module, [Part 05: Using an AWS Marketplace algorithm](/05_Marketplace_alogrithm/05_Marketplace_algorithm.md).  

```r
write_csv(hotels_test, "../data/hotels_test_with_dependent_variable.csv") # Used in the third workshop module

table_header <- hotels_training[NULL, ]
write_csv(table_header, "../data/table_header.csv") # Used in the fifth workshop module
```
