...
# The dataset genertion below needs to be added later! 
hotels_test <- read_csv("../data/hotels_test_with_header.csv")
...
# the code below needs to be updated later!
...
# get SageMaker execution role and AutoGluon ARN stored in .Renviron
role_arn <- Sys.getenv("SAGEMAKER_ROLE_ARN")
autogluon_tabular_arn <- Sys.getenv("AUTOGLUON_TABULAR_ARN")

autogluon_tab_estimator <- sagemaker$AlgorithmEstimator(
  algorithm_arn = autogluon_tabular_arn , 
  role = role_arn, 
  instance_count = 1L, 
...
```{r}
init_args <- list(label = "children", eval_metric = "roc_auc")

fit_args <- list(presets = "best_quality")
...
session$create_model_package_from_algorithm(
  ...
  algorithm_arn = Sys.getenv("AUTOGLUON_TABULAR_ARN"),
  model_data = model_location)



name <- "may-package-01"
# Here we need to use the boto3 client to retrieve the model package ARN, which 
# we need as a parameter when creating the model next. As of the time of writing
# this code the SageMaker Python SDK does not provide a function to retrieve 
# a model package ARN
boto_client <- session$boto_session$client("sagemaker")

model_package_arn <- boto_client$describe_model_package(ModelPackageName = name)[["ModelPackageArn"]]
...
s3_downloader$download(predictions_path, "./predictions")
