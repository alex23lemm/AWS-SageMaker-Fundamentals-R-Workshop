...
The SageMaker deployment process is quite similar to a Batch Transform job you already used twice for generating batch inferences for the hold-out test set in the pervious two workshop modules. However, unlike Batch Transform jobs, a deployment process won't create a short-lived EC2 inference cluster but an inference cluster that is online until the user delibarately decides to shut it down. 

SageMaker executes the following steps automatically once a deplyoment job is started using the `create_endpoint()` function via the API:
...
3. The model endpoint comes live.
...
We will define and save an endpoint configuration which will specify the  


pecifies the ML model, the docker container inference image and the inference cluster configuration.
